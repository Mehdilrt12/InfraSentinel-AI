...
export function Machines() {
  const state = useApi('/machines/', { list: true }); const navigate = useNavigate()
  const columns = [{ key: 'hostname', label: 'Machine' }, { key: 'source_type', label: 'Source', render: (value) => <span className="chip">{value}</span> }, { key: 'ip_address', label: 'Adresse IP' }, { key: 'status', label: 'État', render: (value) => <Status value={value} /> }, { key: 'last_seen', label: 'Dernier contact', render: (value) => value ? new Date(value).toLocaleString('fr-FR') : 'Jamais' }]
  return <Page title="Machines" description="Inventaire centralisé Windows, VMware et Hyper-V."><DataState state={state} empty="Aucune machine n’est encore enrôlée."><Table columns={columns} rows={state.data} onRow={(row) => navigate(`/machines/${row.id}`)} /></DataState></Page>
}

function metricChartData(metrics) {
  const buckets = new Map()
  metrics.slice().reverse().forEach((metric) => { const time = new Date(metric.timestamp).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }); const row = buckets.get(time) || { time }; row[metric.metric_name] = metric.metric_value; buckets.set(time, row) })
  return [...buckets.values()].slice(-60)
}

export function MachineDetail() {
  const { id } = useParams(); const machine = useApi(`/machines/${id}/`); const metrics = useApi(`/metrics/?machine=${id}`, { list: true }); const alerts = useApi(`/alerts/?machine=${id}`, { list: true }); const anomalies = useApi(`/anomalies/?machine=${id}`, { list: true }); const trends = useApi(`/machines/${id}/trends/?hours=24`, { list: true })
  const chart = useMemo(() => metricChartData(metrics.data || []), [metrics.data])
  const metricNames = [...new Set((metrics.data || []).map((item) => item.metric_name))].slice(0, 6)
  return <Page title={machine.data?.hostname || 'Détail machine'} description="Métriques, historique, risque et recommandations contextualisées."><DataState state={machine}>{machine.data && <><div className="detail-strip"><div><Status value={machine.data.status} /><small>État</small></div><div><strong>{machine.data.source_type}</strong><small>Source</small></div><div><strong>{machine.data.ip_address || '—'}</strong><small>Adresse IP</small></div><div><strong>{machine.data.agent_version || 'N/A'}</strong><small>Version agent</small></div></div><section className="panel"><div className="panel-head"><div><h2>Historique normalisé</h2><p>Les séries peuvent avoir des unités différentes ; consultez leur légende et leur unité API.</p></div></div><DataState state={metrics}><div className="chart large"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chart}><CartesianGrid strokeDasharray="3 3" stroke="#183238"/><XAxis dataKey="time" stroke="#77959b"/><YAxis stroke="#77959b"/><Tooltip contentStyle={{ background: '#0c1d22', border: '1px solid #214149' }}/><Legend />{metricNames.map((name, index) => <Area key={name} type="monotone" dataKey={name} stroke={['#24dbc1','#77db64','#54a9ff','#ffb84d','#ab7cff','#ff627d'][index]} fill="none" connectNulls />)}</AreaChart></ResponsiveContainer></div></DataState></section><section className="panel"><h2>Tendances prédictives</h2><p className="muted">Estimations linéaires explicables, jamais présentées comme des certitudes.</p><DataState state={trends} empty="Pas assez d’historique réel pour estimer une tendance."><Table rows={trends.data || []} columns={[{key:'metric_name',label:'Métrique'},{key:'trend',label:'Tendance'},{key:'rate_of_change_per_hour',label:'Variation / h',render:(value)=>formatNumber(value,3)},{key:'risk_score',label:'Risque',render:(value)=>`${value}%`},{key:'confidence',label:'Confiance'},{key:'estimated_threshold_breach_at',label:'Franchissement estimé',render:(value)=>value?new Date(value).toLocaleString('fr-FR'):'Non estimé'}]}/></DataState></section><div className="two-col"><section className="panel"><h2>Alertes et recommandations</h2><DataState state={alerts}>{alerts.data?.slice(0, 5).map((alert) => <article className="event" key={alert.id}><Severity value={alert.severity} /><div><strong>{alert.message}</strong><p>{alert.recommendation || 'Analyse en cours.'}</p></div></article>)}</DataState></section><section className="panel"><h2>Anomalies ML</h2><DataState state={anomalies}>{anomalies.data?.slice(0, 5).map((anomaly) => <article className="event" key={anomaly.id}><BrainCircuit /><div><strong>Score {formatNumber(anomaly.score, 3)}</strong><p>Modèle {anomaly.model_version}</p></div></article>)}</DataState></section></div></>}</DataState></Page>
}
...
export function Anomalies() {
  const state = useApi('/anomalies/', { list: true }); const columns = [{ key: 'hostname', label: 'Machine' }, { key: 'score', label: 'Score', render: (value) => formatNumber(value, 4) }, { key: 'threshold', label: 'Seuil', render: (value) => formatNumber(value, 4) }, { key: 'model_version', label: 'Modèle' }, { key: 'detected_at', label: 'Détectée', render: (value) => new Date(value).toLocaleString('fr-FR') }, { key: 'acknowledged', label: 'Acquittée', render: (value) => value ? <Check /> : 'Non' }]
  return <Page title="Anomalies" description="Événements détectés par les modèles Isolation Forest actifs."><DataState state={state}><Table columns={columns} rows={state.data} /></DataState></Page>
}
...
function AssetDetail({ source }) { const { id }=useParams(); const asset=useApi(`/assets/${id}/`); const machineId=asset.data?.machine; const metrics=useApi(machineId ? `/metrics/?machine=${machineId}` : null,{list:true,enabled:Boolean(machineId)}); return <Page title={asset.data?.name || source} description="Identité, état, parent, métadonnées et métriques réelles."><DataState state={asset}>{asset.data && <><div className="detail-strip"><div><Status value={asset.data.state}/><small>État</small></div><div><strong>{asset.data.kind}</strong><small>Type</small></div><div><strong>{asset.data.parent_external_id || 'Racine'}</strong><small>Parent</small></div></div><section className="panel"><h2>Métadonnées</h2><pre>{JSON.stringify(asset.data.metadata,null,2)}</pre></section><section className="panel"><h2>Mesures</h2><DataState state={metrics}><Table rows={metrics.data.slice(0,100)} columns={[{key:'timestamp',label:'Date',render:(value)=>new Date(value).toLocaleString('fr-FR')},{key:'metric_name',label:'Métrique'},{key:'metric_value',label:'Valeur',render:(value)=>formatNumber(value,2)},{key:'unit',label:'Unité'}]}/></DataState></section></>}</DataState></Page> }
...
export function SettingsPage(){
  const rules=useApi('/rules/',{list:true}); const prefs=useApi('/notifications/preferences/',{list:true}); const environments=useApi('/environments/',{list:true}); const machines=useApi('/machines/',{list:true}); const connectors=useApi('/connectors/',{list:true});
  ...
  async function addRule(event){event.preventDefault();await api.post('/rules/',rule);window.location.reload()} async function addPref(event){event.preventDefault();await api.post('/notifications/preferences/',pref);window.location.reload()}
  async function toggleRule(id){await api.post(`/rules/${id}/toggle/`);window.location.reload()}
